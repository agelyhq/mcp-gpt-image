...
from gpt_image_mcp.config import Settings, get_settings
...
def create_server(settings: Settings | None = None, *, sdk: AsyncOpenAI | None = None) -> FastMCP:
    ...
    deps = ToolDeps(
        images=ImagesClient(settings, sdk=sdk),
        responses=ResponsesImageClient(settings, sdk=sdk),
        store=ImageStore(settings.output_dir),
    )

    # Without an explicit version, a client is told the FastMCP version instead.
    mcp = FastMCP(name="gpt-image", instructions=INSTRUCTIONS, version=version("mcp-gpt-image"))
